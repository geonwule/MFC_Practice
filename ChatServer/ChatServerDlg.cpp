...
void CChatServerDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	//DDX_LBIndex(pDX, IDC_LIST1, m_list);
...
LRESULT CChatServerDlg::OnAccept(WPARAM wParam, LPARAM lParam)
{
	...
	try {
		...
		m_pSocCom[tmp] = new CSocCom();
		// 서버소켓과 통신소켓을 연결한다
		m_pSocCom[tmp] = m_socServer.GetAcceptSocCom();

		m_socServer.m_index.pop_front();
		m_using.push_back(tmp);

		m_pSocCom[tmp]->m_index = tmp;
		m_pSocCom[tmp]->Init(this->m_hWnd);

		// 클라이언트 에게 연결성공 메시지를 보낼때 클라이언트 번호도 같이보냄.
		m_pSocCom[tmp]->Send((_T(SOC_CLIENT_CONNECT) + number), 256);
	}
	catch (CException* ex) {
		ex->ReportError();
	}
...
//데이터를 보내는 것은 소켓클래스의 멤버함수인 Send를 이용
// 데이터를 받을때는 통신소켓 클래스에 오버라이딩한 OnReceive 메시지 함수를 이용
LRESULT CChatServerDlg::OnReceive(WPARAM wParam, LPARAM lParam) {
	...
	if (strTmp.Compare(_T(SOC_CLIENT_DISCONNECT)) == 0) {
		...
		m_list.InsertString(i, _T("사용자") + id + _T(": ") + strTmp);

		// 이 부분 제외하면 서버만 다중 클라이언트로 부터 채팅가능.
		for each (int i in m_using) {
			if (i != _ttoi(id)) {
				// 보낸 클라이언트 제외 모든 클라이언트한테 보냄
				m_pSocCom[i]->Send(_T("사용자") + id + _T(": ") + strTmp, 256);
			}
		}
	}
...
void CChatServerDlg::OnClickedButtonSend()
{
	...
	//pTmp에 전송할 데이터 입력
	memset(pTmp, '\0', 256);
	strcpy_s(pTmp, 256, CT2A(m_strSend));
	m_strSend = "";

	// 전송
	for each (int i in m_using) {
		m_pSocCom[i]->Send(pTmp, 256);
	}
